...
int cts_plat_spi_write(struct cts_platform_data *pdata, u8 i2c_addr,
        const void *src, size_t len, int retry, int delay)
{
    int ret = 0, retries = 0;

	if (len > CFG_CTS_MAX_SPI_XFER_SIZE) {
		cts_err("write too much data:wlen=%d\n", len);
		return -EIO;
	}
	
	pdata->spi_tx_buf[0] = i2c_addr;
...
int cts_plat_spi_read(struct cts_platform_data *pdata, u8 i2c_addr,
        const u8 *wbuf, size_t wlen, void *rbuf, size_t rlen,
        int retry, int delay)
{
    int ret = 0, retries = 0;
	...
	if (wlen > CFG_CTS_MAX_SPI_XFER_SIZE || rlen > CFG_CTS_MAX_SPI_XFER_SIZE) {
		cts_err("write/read too much data:wlen=%d, rlen=%d", wlen, rlen);
		return -EIO;
	}	

	if (pdata->cts_dev->rtdata.program_mode)
	{
		pdata->spi_tx_buf[0] = i2c_addr | 0x01;
...
*/			
		...
		do {
			...
			if (wlen != 0) {
				memcpy(&pdata->spi_tx_buf[1], wbuf, wlen);
		        ret = cts_spi_send_recv(pdata, wlen + 1, pdata->spi_tx_buf, pdata->spi_rx_buf);
				if (ret) {
					if (delay) {
						mdelay(delay);
					}
 			...
 			ret = cts_spi_send_recv(pdata, rlen, pdata->spi_tx_buf, pdata->spi_rx_buf);
			if (ret) {
				if (delay) {
					mdelay(delay);
				}
	...
	if (ret) {
		cts_err("cts_plat_spi_read error,ret=%d", ret);	
	}
...
int cts_plat_is_normal_mode(struct cts_platform_data *pdata)
{
	...
	ret = cts_plat_spi_read(pdata, CTS_DEV_NORMAL_MODE_SPIADDR, tx_buf, 2, rx_buf, 4, 3, 10);
