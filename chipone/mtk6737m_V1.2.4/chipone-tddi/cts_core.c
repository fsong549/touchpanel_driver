...
//        cts_dev->rtdata.i2c_addr, cts_dev->rtdata.addr_width * 2, addr, b);
    ...
    return cts_plat_spi_write(cts_dev->pdata, cts_dev->rtdata.program_mode?
		CTS_DEV_PROGRAM_MODE_SPIADDR:CTS_DEV_NORMAL_MODE_SPIADDR, buff, cts_dev->rtdata.addr_width + 1, retry ,delay);
	return 0;
}
...
//        cts_dev->rtdata.i2c_addr, cts_dev->rtdata.addr_width * 2, addr, w);
    ...
    return cts_plat_spi_write(cts_dev->pdata, cts_dev->rtdata.program_mode? CTS_DEV_PROGRAM_MODE_SPIADDR:CTS_DEV_NORMAL_MODE_SPIADDR,
            buff, cts_dev->rtdata.addr_width + 2, retry, delay);
	return 0;
}
...
//        cts_dev->rtdata.i2c_addr, cts_dev->rtdata.addr_width * 2, addr, len);
    ...
    while (len) {
        ...
        ret = cts_plat_spi_write(cts_dev->pdata, cts_dev->rtdata.program_mode? CTS_DEV_PROGRAM_MODE_SPIADDR:CTS_DEV_NORMAL_MODE_SPIADDR,
                data, xfer_len, retry, delay);
        if (ret) {
            cts_err("Platform i2c write failed %d", ret);
            return ret;
...
//        cts_dev->rtdata.i2c_addr, cts_dev->rtdata.addr_width * 2, addr);
    ...
    return cts_plat_spi_read(cts_dev->pdata, cts_dev->rtdata.program_mode? CTS_DEV_PROGRAM_MODE_SPIADDR:CTS_DEV_NORMAL_MODE_SPIADDR,
            addr_buf, cts_dev->rtdata.addr_width, b, 1, retry, delay);
}
...
//        cts_dev->rtdata.i2c_addr, cts_dev->rtdata.addr_width * 2, addr);
    ...
    ret = cts_plat_spi_read(cts_dev->pdata, cts_dev->rtdata.program_mode? CTS_DEV_PROGRAM_MODE_SPIADDR:CTS_DEV_NORMAL_MODE_SPIADDR,
            addr_buf, cts_dev->rtdata.addr_width, buff, 2, retry, delay);
    if (ret == 0) {
        *w = get_unaligned_le16(buff);
    }
...
int cts_enter_program_mode(struct cts_device *cts_dev)
{
    ...
    if (boot_mode != CTS_DEV_BOOT_MODE_I2C_PROGRAM) {
        cts_err("BOOT_MODE readback %u != I2C_PROMGRAM", boot_mode);
        return -EFAULT;
    }
#else
	if (boot_mode != CTS_DEV_BOOT_MODE_SPI_PROGRAM) {
		cts_err("BOOT_MODE readback %u != SPI_PROGRAM", boot_mode);
		return -EFAULT;
	}
#endif
