...
static int chipone_tpd_local_init(void)
{
	...
	if (ret) {
		cts_err("regulator_set_voltage(%d) failed!\n", ret);
		return -1;
	}
...
static ssize_t cts_show_chipinfo(struct device *dev,struct device_attribute *attr, char *buf)
{	
	ssize_t res;
	res = scnprintf(buf,PAGE_SIZE, "chipinfo:%s\n",CFG_CTS_DRIVER_NAME);
	return res;
}
static DEVICE_ATTR(chipinfo,  0444, cts_show_chipinfo,NULL);

static struct device_attribute *cts_attrs[] = {
			&dev_attr_chipinfo,
		};
...
static struct tpd_driver_t chipone_tpd_driver = {
    ...
    .tpd_have_button = 0,
	.attrs.attr = cts_attrs,
	.attrs.num = sizeof(cts_attrs)/sizeof(cts_attrs[0])
