...
int  icn87xx_read_flashid(void)
{
   ...
   unsigned char ucTemp[4];
	...
	flash_info("icnt87 icn87xx_read_flashid: 0x%x,0x%x,0x%x\n",ucTemp[0],ucTemp[1],ucTemp[2]);
    if(ret < 0)
    {
        return ret;
    }
...
int  icn87xx_fw_update(void *fw)
{
    ...
    {
    	 printk("icn87xx_erase_chip:start  \n");
		 ...
		 printk("icn87xx_erase_chip:over  \n");
    }
    ...
    if(ICN85XX_WITH_FLASH_87 == boot_mode)
    {
        ...
        printk("icn87xx_boot_flash start\n");
		j = 10;
        while(version != 0x81 && j--)
        {
		...
		printk("icn87xx_boot_flash over\n");
