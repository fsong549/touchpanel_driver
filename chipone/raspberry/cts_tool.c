...
#define BigLittleSwap32(A)        ((((u32)(A) & 0xff000000) >> 24) | \
    ...
    cts_info("retry   :%d.\n", cmd->retry);
    cts_info("data len:%d.\n", cmd->data_len);
	cts_info("data len:%d.\n", BigLittleSwap32(cmd->data_len));
	cts_info("0x12345678:%d.\n", BigLittleSwap32(0x12345678));
    cts_info("addr len:%d.\n", cmd->addr_len);
    cts_info("addr:0x%02x%02x.\n", cmd->addr[0], cmd->addr[1]);
    cts_info("len:%d.\n", (int)count);
    cts_info("data:0x%02x%02x.\n", buffer[CTS_TOOL_CMD_HEADER_LENGTH], buffer[CTS_TOOL_CMD_HEADER_LENGTH+1]);

	if(cmd->cmd == 1)
	{
		ret = cts_get_fwid(&cts_data->cts_dev, &fwid);
			   if (ret) {
				   cts_err("Get firmware id failed %d, retries %d", ret, 3);
			   }
		cts_info("fwid:%d.\n", fwid);
	}
	else if(cmd->cmd == 2)
	{
		ret = cts_fw_reg_readw_retry(&cts_data->cts_dev,
