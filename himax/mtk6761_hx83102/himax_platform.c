...
extern unsigned touch_ver;
extern unsigned display_ver;
...
static int hx_panel_coords[4] = {0, 640, 0, 1352}; /* [1]=X resolution, [3]=Y resolution */
static int hx_display_coords[4] = {0, 640, 0, 1352};
...
#endif
//add for 9375 info start
#if defined (SAMPLE_DISPLAY_VERSION) && defined (SAMPLE_TOUCH_VERSION)       
len=sprintf(tp_info,"TP IC: HX%d,TP MODULE: %s,TP I2C ADR: 0x%x,SW Display_FirmWare: 0x%03X,SW TP_FirmWare: 0x%03X,Sample Display_FirmWare: 0x%03X,Sample TP_Firmware: 0x%03X",driver_ic,"Himax",client->addr,display_ver,touch_ver,SAMPLE_DISPLAY_VERSION,SAMPLE_TOUCH_VERSION);
#else       
len=sprintf(tp_info,"TP IC: HX%d,TP MODULE: %s,TP I2C ADR: 0x%x,SW Display_FirmWare: 0x%03X,SW TP_FirmWare: 0x%03X,",driver_ic,"Himax",client->addr,ic_data->vendor_display_cfg_ver,ic_data->vendor_touch_cfg_ver);	
#endif           
