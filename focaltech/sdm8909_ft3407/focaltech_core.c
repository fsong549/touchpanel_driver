...
static irqreturn_t fts_ts_interrupt(int irq, void *dev_id)
{
	...
	disable_irq_wake(fts_ts->client->irq);//tp gesture wakeup
	if (!fts_ts) {
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	for (i = 0; i < event->touch_point; i++)
	{
		...
		if (event->au8_touch_event[i] == FTS_TOUCH_DOWN || event->au8_touch_event[i] == FTS_TOUCH_CONTACT)
		{
			...
			/* Moidfied by JZZ(zhizhang) for TP can not work if touch the TP when power on 20161028 */
			...
			//pr_err("*****Enter:%s line = %d  event->au16_x[i] = %d    event->au16_y[i] = %d\n",__func__,__LINE__,event->au16_x[i],event->au16_y[i]);//zhangbing add			
		}
		else
		{
			uppoint++;
	...
	if (on) {
		if (gpio_is_valid(data->pdata->irq_gpio)) {
			err = gpio_request(data->pdata->irq_gpio,
						"focaltech,irq-gpio");//"fts_irq_gpio"
			if (err) {
				dev_err(&data->client->dev,
		...
		if (gpio_is_valid(data->pdata->reset_gpio)) {
			err = gpio_request(data->pdata->reset_gpio,
						"focaltech,reset-gpio");//"fts_reset_gpio"
			if (err) {
				dev_err(&data->client->dev,
...
#ifdef UNISCOPE_DRIVER_QC8909  //liguowei@uniscope.com 20140703
#define fts_VDD_LOAD_MAX_UA	10000 //liguowei
...
static int fts_power_off(struct fts_ts_data *data)
{
	int ret;
	if (data->power_on==0) {
		dev_err(&data->client->dev,
...
#ifdef UNISCOPE_DRIVER_QC8909  //liguowei@uniscope.com 20140703
static int fts_power_init(struct fts_ts_data *data )
{
	int ret;
...
int fts_ts_suspend(struct device *dev)
{
	...
	#if FTS_GESTRUE_EN
	//pr_err("*****Enter::::::::::suhongzhu:::::::::suspend:::::::::::::::::li\n");//zhangbing add
...
#ifdef UNISCOPE_DRIVER_QC8909  //liguowei@uniscope.com 20140703
	data->power_on = false;
	err = fts_power_init(data);
	if (err) {
		dev_err(&client->dev, "fts power init failed\n");
			goto unreg_inputdev;
	}

	err = fts_power_on(data);
	if (err) {
		dev_err(&client->dev, "fts power on failed\n");
			goto pwr_deinit;
	}
#else
...
//
	...
	FTS_STORE_TS_INFO(data->ts_info, data->family_id, data->pdata->name,
		...
		fts_Gesture_init(input_dev);
		 if (fts_updateinfo_curr.CHIP_ID != 0x54 &&  fts_updateinfo_curr.CHIP_ID != 0x58 &&  fts_updateinfo_curr.CHIP_ID != 0x86 &&  fts_updateinfo_curr.CHIP_ID != 0x87)
		 {
			init_para(480,854,0,0,0);//zhangbing add
		 }
	#endif
	...
	*/
	...
	//pr_err("*****Enter:%s line = %d   ft3407  probe successfully!\n",__func__,__LINE__);//zhangbing add
	return 0;
