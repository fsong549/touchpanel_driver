...
static unsigned char CTPM_FW[] = {
	...
	#include "LQ_CM280_FT5346_Ofilm0x51_V04_D01_20151217_app"
};
...
static unsigned char ft5x46_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			...
			FTS_DBG("[FTS] Step 3 ok: CTPM ID,ID1 = 0x%x,ID2 = 0x%x ,0x%x, 0x%x:\n",
	...
	if (i >= FTS_UPGRADE_LOOP)
	{
		FTS_DBG("[FTS] FTS_UPGRADE_LOOP is  i = %d \n", i);
		return -EIO;
	}
	FTS_DBG("[FTS] OK: FTS_UPGRADE_LOOP is  i = %d \n", i);
	auc_i2c_write_buf[0] = 0xcd;
	fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 1);
	FTS_DBG("[FTS]bootloader version = 0x%x\n", reg_val[0]);
	auc_i2c_write_buf[0] = 0x03;
	auc_i2c_write_buf[1] = 0x00;
	for(i = 0;i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		FTS_DBG("%s: REG VAL ID1 = 0x%x,ID2 = 0x%x\n", __func__,reg_val[0],reg_val[1]);	
		break;
