...
#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
u8 g_EnableTpProximity = 0;
u8 g_EnableTpProximity_failed=0;
...
//#define ENABLE_GESTURE_WAKEUP_FOR_TEST_KEYPOWER  //BIRD ADD
        ...
        if (bIsCorrectFormat) 
        {
            ...
            switch (nWakeupMode)
            {
...
#ifdef ENABLE_GESTURE_WAKEUP_FOR_TEST_KEYPOWER
                    input_report_key(g_InputDevice, KEY_POWER, 1);
                    input_sync(g_InputDevice);
                    input_report_key(g_InputDevice, KEY_POWER, 0);
                    input_sync(g_InputDevice);
                ...
                case 0x61:
                    ...
                    input_sync(g_InputDevice);
#else
                    input_report_key(g_InputDevice, KEYCODE_CTPDOWN, 1);
                    input_sync(g_InputDevice);
                    input_report_key(g_InputDevice, KEYCODE_CTPDOWN, 0);
                    input_sync(g_InputDevice);
#endif
                    break;		
                case 0x62:
...
//                    input_report_key(g_InputDevice, RESERVER3, 0);
                    ...
                    break;
#endif
#if 0//def CONFIG_SUPPORT_64_TYPES_GESTURE_WAKEUP_MODE
                case 0x6F:
...
/*Add by bird begin*/
if ((g_EnableTpProximity_failed == 1) ||(g_EnableTpProximity == 1))
    {
        DrvDisableProximity();
		mdelay(100);
        DBG(&g_I2cClient->dev, "g_EnableTpProximity_failed = %d\n", g_EnableTpProximity_failed);
	 DrvEnableProximity();
        return;
    }
/*Add by bird end*/
...
//add for tp info begin by swb 20171207  
    ...
    sprintf(fw_version, "[FW]0x%x,0x%x [IC]MSG2238",nMajor,nMinor);
