...
#include "include/config0/gt1x_config.h"
...
#ifdef CONFIG_GTP_CHARGER_SWITCH
	const u8 cfg_grp0_charger[] = GTP_CHARGER_CFG_GROUP0;
	const u8 cfg_grp1_charger[] = GTP_CHARGER_CFG_GROUP1;
	const u8 cfg_grp2_charger[] = GTP_CHARGER_CFG_GROUP2;
	const u8 cfg_grp3_charger[] = GTP_CHARGER_CFG_GROUP3;
	const u8 cfg_grp4_charger[] = GTP_CHARGER_CFG_GROUP4;
	const u8 cfg_grp5_charger[] = GTP_CHARGER_CFG_GROUP5;
...
s32 gt1x_init(void)
{
	...
	while (retry++ < 5) {
		...
		snprintf(xc_tp_info, sizeof(xc_tp_info),"ChipId:GT5688\n");
