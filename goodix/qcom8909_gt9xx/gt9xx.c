...
/*liguowei@uni	20161024  pinctrl  used by touchscreen begin */
#define PINCTRL_STATE_ACTIVE	"pmx_ts_active"
#define PINCTRL_STATE_SUSPEND	"pmx_ts_suspend"
#define PINCTRL_STATE_RELEASE	"pmx_ts_release"
/*liguowei@uni	20161024  pinctrl  used by touchscreen end */
...
extern bool STK3311_PS_ENABLE_TO_GT9157; //liguowei@uni_drv L501 disney LCD can not wakeup when calling
...
#if GTP_GESTURE_WAKEUP
static int gesture=1;   //zhangbing@uniscope_drv 20151218 modify
#endif
/*******************************************************
Function:
...
void gtp_irq_disable(struct goodix_ts_data *ts)
{
    ...
    {
        ts->irq_is_disable = 1; 
	 disable_irq_wake(ts->client->irq);//zhangbing@uniscope_drv 20151203 add in case of cannot wakeup when deep sleep
...
void gtp_irq_enable(struct goodix_ts_data *ts)
{
    ...
    if (ts->irq_is_disable) 
    {
	 ...
	 enable_irq_wake(ts->client->irq);//zhangbing@uniscope_drv 20151203 add in case of cannot wakeup when deep sleep
        ts->irq_is_disable = 0; 
    }
    spin_unlock_irqrestore(&ts->irq_lock, irqflags);
}
...
#else
    input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, -1);//zhangbing@uniscope_drv 20161222 add for TP work abnomal at ffbm mode
...
#if GTP_ICS_SLOT_REPORT
    //input_mt_init_slots(ts->pen_dev, 16);  // zhangbing modify
    input_mt_init_slots(ts->pen_dev, 10, 0);
#else
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    {               
        ...
        if (ret > 0)
        {     /*zhangbing@uniscope_drv 20151203 modify gesture begin*/
            if ( (doze_buf[2] == 'c') || (doze_buf[2] == 'e') || (doze_buf[2] == 'm') ||
		    (doze_buf[2] == 'o') || (doze_buf[2] == 'v')|| (doze_buf[2] == 'w')|| (doze_buf[2] == 'z')|| (doze_buf[2] == 's'))  //suhongzhu@uni_drv 1220 add gesture w/z/s for L560S
               // (doze_buf[2] == 0x5E) /* ^ */
            {/*
                if (doze_buf[2] != 0x5E)
                {
		   ...
		   if(doze_buf[2] == 'c')
		   {
		      if(STK3311_PS_ENABLE_TO_GT9157==0)//liguowei@uni_drv L501 disney LCD can not wakeup when calling
		      	{
			...
			gtp_enter_doze(ts);//suhongzhu@uni_drv 0221 solve one gesture don't work cause other gesture can't work
			/*
			mdelay(20);
		   ...
		   else if(doze_buf[2] == 'e')
		   {
		     if(STK3311_PS_ENABLE_TO_GT9157==0)//liguowei@uni_drv L501 disney LCD can not wakeup when calling
		      	{
		   	GTP_INFO(" 'e' to light up the screen!\n");
		   ...
		   else if(doze_buf[2] == 'o')
		   {
		   	...
		   	input_report_key(ts->input_dev,KEY_GESTURE_O, 1);
			input_sync(ts->input_dev);
			input_report_key(ts->input_dev,KEY_GESTURE_O, 0);
			input_sync(ts->input_dev);
		   	}
			doze_buf[2] = 0x00;
			gtp_i2c_write(i2c_connect_client, doze_buf, 3);
		   ...
		   /*suhongzhu@uni_drv 1220 add gesture w/z/s for L560S begin*/
		   else if(doze_buf[2] == 'w')
		   {
		   	if(STK3311_PS_ENABLE_TO_GT9157==0)//liguowei@uni_drv L501 disney LCD can not wakeup when calling
		      	{
		   	GTP_INFO(" 'w' to light up the screen!\n");
		   	input_report_key(ts->input_dev,KEY_GESTURE_W, 1);//KEY_GESTURE_W
			input_sync(ts->input_dev);
		...
		else if(doze_buf[2] == 'z')
		   {
		   	if(STK3311_PS_ENABLE_TO_GT9157==0)//liguowei@uni_drv L501 disney LCD can not wakeup when calling
		      	{
		   	GTP_INFO(" 'w' to light up the screen!\n");
		   	input_report_key(ts->input_dev,KEY_GESTURE_Z, 1);//KEY_GESTURE_Z
			input_sync(ts->input_dev);
		...
		else if(doze_buf[2] == 's')
		   {
		   	if(STK3311_PS_ENABLE_TO_GT9157==0)//liguowei@uni_drv L501 disney LCD can not wakeup when calling
		      	{
		   	GTP_INFO(" 'w' to light up the screen!\n");
		   	input_report_key(ts->input_dev,KEY_GESTURE_S, 1);//KEY_GESTURE_S
			input_sync(ts->input_dev);
		   ...
		   /*suhongzhu@uni_drv 1220 add gesture w for L560S end*/
		}
		/*suhongzhu@uni_drv 1220 open gesture left/down/right/up for L560S begin*/
	#if 1 	
		   ...
		   if(type == 0)
		   {
		   	GTP_INFO("slide right to light up the screen!\n");
		   	input_report_key(ts->input_dev, KEY_GESTURE_RIGHT, 1);
                	input_sync(ts->input_dev);
                	input_report_key(ts->input_dev, KEY_GESTURE_RIGHT, 0);
                	input_sync(ts->input_dev);
               	 // clear 0x814B
                	doze_buf[2] = 0x00;
                	gtp_i2c_write(i2c_connect_client, doze_buf, 3);
		   ...
		   if(type == 1)
		   {
		   	...
		   	input_report_key(ts->input_dev, KEY_GESTURE_DOWN, 1);
                	input_sync(ts->input_dev);
                	input_report_key(ts->input_dev, KEY_GESTURE_DOWN, 0);
                	input_sync(ts->input_dev);
               	 // clear 0x814B
                	doze_buf[2] = 0x00;
                	gtp_i2c_write(i2c_connect_client, doze_buf, 3);
		   ...
		   else if(type == 2)
		   {
		   	...
		   	input_report_key(ts->input_dev, KEY_GESTURE_UP, 1);
                	input_sync(ts->input_dev);
                	input_report_key(ts->input_dev, KEY_GESTURE_UP, 0);
                	input_sync(ts->input_dev);
               	 // clear 0x814B
                	doze_buf[2] = 0x00;
                	gtp_i2c_write(i2c_connect_client, doze_buf, 3);
		   ...
		   {
		   	GTP_INFO("slide left to light up the screen!\n");
		   	input_report_key(ts->input_dev, KEY_GESTURE_LEFT, 1);
                	input_sync(ts->input_dev);
                	input_report_key(ts->input_dev, KEY_GESTURE_LEFT, 0);
                	input_sync(ts->input_dev);
               	 // clear 0x814B
                	doze_buf[2] = 0x00;
                	gtp_i2c_write(i2c_connect_client, doze_buf, 3);
	...
	#endif  
		/*suhongzhu@uni_drv 1220 open gesture left/down/right/up for L560S end*/
            	 ...
            	 if(STK3311_PS_ENABLE_TO_GT9157==0)//liguowei@uni_drv L501 disney LCD can not wakeup when calling
		  {
                ...
                doze_status = DOZE_WAKEUP;
                input_report_key(ts->input_dev, KEY_DOUBLE_CLICK, 1);
                input_sync(ts->input_dev);
                input_report_key(ts->input_dev, KEY_DOUBLE_CLICK, 0);
                input_sync(ts->input_dev);
            	   }
                // clear 0x814B
                doze_buf[2] = 0x00;
		...
		gtp_enter_doze(ts);//suhongzhu@uni_drv 0221 solve one gesture don't work cause other gesture can't work
			...
			/*zhangbing@uniscope_drv 20151203 modify gesture end*/
            else
            {
                // clear 0x814B
                doze_buf[2] = 0x00;
...
#ifdef	GTP_CONFIG_OF    //zhangbing@uniscope_drv 20160616 add for TP parameter
...
static ssize_t gt91xx_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    ...
    GTP_DEBUG("write count %ul\n", count);

    if (count > GTP_CONFIG_MAX_LENGTH)
    {
        GTP_ERROR("size not match [%d:%ul]\n", GTP_CONFIG_MAX_LENGTH, count);
        return -EFAULT;
    }
...
#if GTP_GESTURE_WAKEUP
    input_set_capability(ts->input_dev, EV_KEY, KEY_DOUBLE_CLICK);  
/*zhangbing@uniscope_drv 20151204 add begin*/
    ...
    input_set_capability(ts->input_dev, EV_KEY, KEY_POWER);//suhongzhu add gesture adjust
    input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_W);//suhongzhu add gesture adjust
    input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_LEFT);//suhongzhu add gesture adjust
    input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_DOWN);//suhongzhu add gesture adjust
    input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_RIGHT);//suhongzhu@uni_drv 20170210 add gesture for L560S
    input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_UP);//suhongzhu@uni_drv 20170210 add gesture for L560S
    input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_Z);//suhongzhu@uni_drv 20170210 add gesture for L560S
    input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_S);//suhongzhu@uni_drv 20170210 add gesture for L560S
/*zhangbing@uniscope_drv 20151204 add end*/
#endif 
...
static void gtp_parse_dt(struct device *dev)
{
	...
	gtp_rst_gpio = of_get_named_gpio(np, "reset-gpios", 0);//zhangbing@uniscope_drv 20151203 modify to match the DeviceTree
	gtp_int_gpio = of_get_named_gpio(np, "interrupt-gpios", 0);
}
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (!avdd) {
		...
		if (IS_ERR(avdd)) {
			GTP_ERROR("regulator get of avdd failed");
			ret = PTR_ERR(avdd);
			avdd = NULL;
			return ret;
		}
...
//support GT615 and MSG5648 IC 20160822 begin
	GTP_GPIO_FREE(gtp_rst_gpio);
	GTP_GPIO_FREE(gtp_int_gpio);
	kfree(ts); 
	return ret; 
//support GT615 and MSG5648 IC 20160822 
    }
